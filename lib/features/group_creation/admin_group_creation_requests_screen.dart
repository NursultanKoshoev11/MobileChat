...
class _AdminGroupCreationRequestsScreenState
  ...
  Future<void> refresh() async {
    final nextFuture =
        widget.api.fetchAdminGroupCreationRequests(status: status);
    setState(() {
      future = nextFuture;
    });
    await nextFuture;
  ...
  Future<void> changeStatus(String value) async {
    final nextFuture =
        widget.api.fetchAdminGroupCreationRequests(status: value);
    setState(() {
      status = value;
      future = nextFuture;
    });
  ...
  Future<void> review(GroupCreationRequest request, String action) async {
    ...
    try {
      if (action == 'approve') {
        await widget.api
            .approveGroupCreationRequest(request.id, adminComment: comment);
      } else if (action == 'reject') {
        await widget.api
            .rejectGroupCreationRequest(request.id, adminComment: comment);
      } else {
        await widget.api.needMoreInfoForGroupCreationRequest(request.id,
            adminComment: comment);
      }
      await refresh();
      if (mounted)
        showAppSnack(
            context, text.isKy ? 'Өтүнүч жаңыртылды.' : 'Заявка обновлена.');
    } catch (e) {
          ...
          SingleChildScrollView(
            ...
            child: SegmentedButton<String>(
              ...
              segments: [
                ...
                ButtonSegment(
                    value: '', label: Text(text.isKy ? 'Баары' : 'Все')),
              ],
              selected: {status},
                ...
                builder: (context, snapshot) {
                  ...
                  return ListView.builder(
                    ...
                    itemBuilder: (context, index) {
                      ...
                      final canReview = request.status == 'pending' ||
                      ...
                      return GroupCreationRequestCard(
                        ...
                        actions: canReview
                            ? Wrap(spacing: 8, runSpacing: 8, children: [
                                FilledButton.icon(
                                    onPressed: () => review(request, 'approve'),
                                ...
                                OutlinedButton.icon(
                                    ...
                                    label: Text(text.isKy
                                        ...
                                        : 'Нужна информация')),
                                OutlinedButton.icon(
                                    onPressed: () => review(request, 'reject'),

      ...
      body: RefreshIndicator(
        ...
        child: FutureBuilder<List<ChatGroup>>(
          ...
          builder: (context, snapshot) {
            ...
            if (groups.isEmpty)
              return _EmptyGroups(isAdmin: isAdmin, onCreate: createGroup);
            return ListView.builder(
              padding: const EdgeInsets.fromLTRB(16, 8, 16, 96),
              itemCount: groups.length,
              itemBuilder: (_, index) => GroupTile(
                  key: ValueKey('group_tile_${groups[index].id}'),
                  group: groups[index],
                  onTap: () => openGroup(groups[index]),
                  onLeave: () => leaveGroup(groups[index])),
            );
          },
        ),
      ),
...
class _MenuItem extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return ListTile(
      ...
      title: Text(title),
      trailing:
          count != null && count! > 0 ? Badge(label: Text('$count')) : null,
      onTap: () {
            ...
            PopupMenuButton<String>(
              ...
              itemBuilder: (_) => [
                if (inviteCode.isNotEmpty)
                  PopupMenuItem(
                      value: 'copy', child: Text(text.copyInviteCode)),
                PopupMenuItem(
                    value: 'leave',
                    child:
                        Text(text.isKy ? 'Топтон чыгуу' : 'Выйти из группы')),
              ],
            ),
          ]),
        ),
